...
function Metric({
  ...
  return (
    <div className="min-w-0 rounded-[5px] border border-edge bg-[#0c1424] px-2 py-1.5">
      <div className="truncate text-[8px] font-semibold uppercase tracking-[0.09em] text-[#6d7f9e]">{label}</div>
      <div className={`tnum mt-[2px] truncate text-[13px] font-bold leading-tight ${valueTone}`}>{value}</div>
      {sub ? <div className="truncate text-[8px] text-[#7f93b3]">{sub}</div> : null}
    </div>
  );
}
...
function Attribute({ label, value, confidence }: { label: string; value: string; confidence: number }) {
  ...
  return (
    <div className="flex items-center gap-1.5">
      <span className="w-[74px] shrink-0 truncate text-[8.5px] uppercase tracking-[0.06em] text-[#6d7f9e]">{label}</span>
      <span className="min-w-0 flex-1 truncate text-[9.5px] font-medium text-[#dbe6f5]">{value}</span>
      ...
      <span className="tnum w-[26px] shrink-0 text-right text-[8px] text-[#7f93b3]">{confidence}%</span>
    </div>
  );
}
    ...
    <Panel
      ...
      tools={
        <div className="flex items-center gap-1.5">
          ...
          <span
            className={`flex items-center gap-1 rounded-[4px] border px-1.5 py-[2px] text-[8.5px] font-bold uppercase tracking-[0.07em] ${
              watchlist.match
    ...
    >
      <div className="flex items-stretch gap-3">
        ...
        >
          ...
          <span className="absolute left-1.5 top-1.5 flex items-center gap-1 rounded-[3px] bg-black/75 px-1.5 py-px text-[8px] font-bold text-[#9fb0cc] ring-1 ring-edge-strong">
          ...
          <span className="tnum absolute right-1.5 top-1.5 rounded-[3px] bg-black/75 px-1.5 py-px text-[8px] font-semibold text-[#c9d6ea] ring-1 ring-edge-strong">
            {last.cameraId} · {last.time}
          </span>
          <span className="absolute bottom-1.5 left-1.5 right-1.5 flex items-end justify-between gap-1">
            <span className="tnum rounded-[3px] bg-[#2a0d13]/90 px-1.5 py-px font-mono text-[10px] font-bold tracking-[0.08em] text-white ring-1 ring-accent-red/50">
              {target.plate}
            </span>
            <span className="flex items-center gap-1 text-[8px] text-[#8ea1c0] opacity-0 transition-opacity group-hover:opacity-100">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start gap-2">
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                ...
                <span className="rounded-[4px] border border-edge-strong bg-[#16233a] px-1.5 py-[2px] text-[9px] font-semibold uppercase tracking-[0.06em] text-[#c3cfe2]">
                  {target.color}
                </span>
                <span className="rounded-[4px] border border-edge bg-[#0c1424] px-1.5 py-[2px] text-[9px] font-semibold uppercase tracking-[0.06em] text-[#9fb0cc]">
                  {target.vehicleClass}
                </span>
              </div>
              <div className="mt-1 flex items-center gap-1.5 text-[11px] font-semibold text-[#dbe6f5]">
                <IdCard size={11} className="shrink-0 text-accent-cyan" />
                {target.label}
                <span className="text-[9.5px] font-normal text-[#7f93b3]">
            ...
            <div className="ml-auto flex shrink-0 items-center gap-1.5">
              <button
                ...
                className="flex h-[26px] items-center gap-1 rounded-[4px] border border-edge bg-[#0c1424] px-2 text-[9.5px] font-semibold text-[#c3cfe2] transition-colors hover:border-accent-blue/60 hover:text-white"
              >
                <CameraIcon size={11} />
                {last.cameraId}
              </button>
              <button
                ...
                className="flex h-[26px] items-center gap-1 rounded-[4px] border border-accent-red/45 bg-accent-red/10 px-2 text-[9.5px] font-semibold text-[#ff8b96] transition-colors hover:border-accent-red/70 hover:bg-accent-red/20"
              >
                <ShieldAlert size={11} />
                {watchlist.entryId}
              </button>
          ...
          <div className="mt-2 grid grid-cols-2 gap-x-3 gap-y-[3px] rounded-[5px] border border-edge bg-[#0c1424] px-2 py-1.5">
            <div className="flex items-center gap-1.5">
              ...
              <span className="truncate text-[8.5px] uppercase tracking-[0.06em] text-[#6d7f9e]">Owner</span>
              <span className="truncate text-[9.5px] font-medium text-[#dbe6f5]">{target.registeredOwner}</span>
            </div>
            <div className="flex items-center gap-1.5">
              <span className="truncate text-[8.5px] uppercase tracking-[0.06em] text-[#6d7f9e]">Registration</span>
              <span className="truncate text-[9.5px] font-medium text-[#dbe6f5]">{target.registrationState}</span>
            </div>
            <div className="flex items-center gap-1.5">
              <span className="truncate text-[8.5px] uppercase tracking-[0.06em] text-[#6d7f9e]">Insurance</span>
              <span className="truncate text-[9.5px] font-medium text-[#f7b95f]">{target.insuranceExpiry}</span>
            </div>
            <div className="flex items-center gap-1.5">
              <span className="truncate text-[8.5px] uppercase tracking-[0.06em] text-[#6d7f9e]">Fitness</span>
              <span className="truncate text-[9.5px] font-medium text-[#dbe6f5]">{target.fitnessExpiry}</span>
            </div>
        ...
        <div className="w-[228px] shrink-0">
          ...
          <div className="mt-1.5 rounded-[5px] border border-edge bg-[#0c1424] px-2 py-1.5">
            <div className="flex items-center justify-between">
              <span className="flex items-center gap-1 text-[8.5px] font-semibold uppercase tracking-[0.09em] text-[#6d7f9e]">
                <Crosshair size={10} className="text-accent-cyan" />
                Watchlist status
              </span>
              <span className="tnum text-[8px] text-[#7f93b3]">added {watchlist.addedOn}</span>
            </div>
            <div className="mt-1 flex items-center gap-1.5">
              <span className="rounded-[3px] bg-accent-red/20 px-1.5 py-px text-[8.5px] font-bold uppercase tracking-[0.06em] text-[#ff8b96] ring-1 ring-accent-red/40">
                {watchlist.category}
              </span>
              <span className="tnum rounded-[3px] bg-[#16233a] px-1.5 py-px text-[8px] text-[#9fb0cc]">{watchlist.entryId}</span>
              <span className="tnum rounded-[3px] bg-[#16233a] px-1.5 py-px text-[8px] uppercase text-[#9fb0cc]">
                {watchlist.priority}
              </span>
            </div>
            <p className="mt-1 text-[8.5px] leading-[12px] text-[#94a5c2]">{watchlist.action}</p>
          </div>

          <div className="mt-1.5 flex items-center gap-1.5">
            <span className="flex flex-1 items-center gap-1 rounded-[4px] border border-edge bg-[#0c1424] px-1.5 py-1 text-[8.5px] text-[#94a5c2]">
            ...
            <span className="tnum flex items-center gap-1 rounded-[4px] border border-edge bg-[#0c1424] px-1.5 py-1 text-[8.5px] text-[#94a5c2]">
