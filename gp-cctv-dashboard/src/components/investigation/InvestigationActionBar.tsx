...
const ghostBtn =
  'flex h-[30px] items-center gap-1.5 rounded-[5px] border border-edge bg-panel px-2.5 text-[12px] font-semibold text-[#c3cfe2] transition-colors hover:border-edge-strong hover:text-white disabled:cursor-not-allowed disabled:opacity-40';
  ...
  return (
    <div className="sticky bottom-0 z-30 -mx-3 mt-auto flex shrink-0 items-center justify-between gap-3 border-t border-edge bg-[#070c17]/95 px-3 py-2 backdrop-blur">
      <div className="flex min-w-0 items-center gap-2">
        <span className="tnum rounded-[4px] border border-edge bg-panel px-1.5 py-1 text-[11px] font-semibold text-[#c3cfe2]">
          {dossier.caseId}
        </span>
        <span className="tnum truncate font-mono text-[13px] font-bold tracking-[0.06em] text-white">
          {dossier.target.plate}
        </span>
        <span className="truncate text-[11px] text-[#7f93b3]">
          {dossier.sightings.length} sightings · {evidenceCount} evidence frames · last read {lastCamera}
          {caseRef ? ` · case ${caseRef}` : ' · case not filed'}
        </span>
        <span
          className={`shrink-0 rounded-[3px] px-1.5 py-px text-[10.5px] font-bold uppercase tracking-[0.07em] ring-1 ${
            closed
