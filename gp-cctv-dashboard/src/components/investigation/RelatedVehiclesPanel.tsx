        ...
        return (
          ...
          >
            <span className="relative h-[42px] w-[58px] shrink-0 overflow-hidden rounded-[4px] border border-edge-soft">
              ...
              <span
                className={`absolute inset-x-0 bottom-0 flex items-center justify-center gap-0.5 bg-black/75 py-px text-[9px] font-bold uppercase tracking-[0.06em] ${toneRing[association.tone].split(' ')[1]}`}
              >
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-1.5">
                ...
                <span className="shrink-0 rounded-[3px] bg-[#16233a] px-1 py-px text-[9.5px] font-semibold uppercase tracking-[0.06em] text-[#9fb0cc]">
              ...
              <div className="mt-1 flex items-center gap-1.5">
                {association.kind === 'registered-owner' ? (
                  <span className="flex items-center gap-1 text-[10px] text-[#7f93b3]">
                ...
                <span className="ml-auto flex items-center gap-1">
                  <button
                    ...
                    className="flex h-[19px] items-center gap-1 rounded-[3px] border border-edge bg-[#0d1626] px-1.5 text-[10.5px] font-semibold text-[#9fc7ff] transition-colors hover:border-accent-red/60 hover:text-[#ff8b96]"
                  >
                    <ShieldAlert size={9} />
                    Watchlist
                  </button>
                  <button
                    ...
                    className="flex h-[19px] items-center gap-1 rounded-[3px] border border-accent-blue/45 bg-[#12233f] px-1.5 text-[10.5px] font-semibold text-[#9fc7ff] transition-colors hover:border-accent-blue/70 hover:text-white disabled:cursor-not-allowed disabled:opacity-40"
                  >
                    <FolderOpen size={9} />
                    Dossier
                  </button>
              ...
              {association.sightings.length ? (
                <div className="mt-1 flex flex-wrap gap-1">
                  {association.sightings.map((code) => (
                    <span key={code} className="tnum rounded-[3px] border border-edge bg-[#0d1626] px-1 py-px text-[9.5px] text-[#7f93b3]">
                      {code}
                    </span>
                  ))}
                </div>
