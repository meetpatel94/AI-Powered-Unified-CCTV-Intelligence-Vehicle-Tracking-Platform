  ...
  return (
      ...
      tools={
        <div className="flex items-center gap-1.5">
          ...
          <div className="flex items-center gap-px overflow-hidden rounded-[4px] border border-edge bg-[#0a1120] p-px">
            {filters.map((item) => (
              <button
                ...
                className={`h-[20px] px-2 text-[9px] font-semibold transition-colors ${
          ...
          <select
            ...
            className="h-[22px] rounded-[4px] border border-edge bg-[#0c1424] px-1.5 text-[9px] text-[#c3cfe2] outline-none transition-colors hover:border-edge-strong focus:border-accent-blue/70"
          >
    ...
    >
      <div className="grid h-full min-h-0 grid-cols-[repeat(auto-fill,minmax(158px,1fr))] gap-1.5 overflow-y-auto">
        {evidence.map((item) => (
          ...
          >
            ...
            >
              ...
              <span className="tnum absolute left-1 top-1 rounded-[2px] bg-black/75 px-1 py-px font-mono text-[8px] font-bold text-white">
                {item.cameraId}
              </span>
              <span className="tnum absolute right-1 top-1 rounded-[2px] bg-black/75 px-1 py-px text-[7.5px] text-[#c9d6ea]">
                {item.confidence.toFixed(1)}%
              </span>
              <span className="absolute bottom-1 left-1 flex items-center gap-1 rounded-[2px] bg-black/75 px-1 py-px text-[7px] font-semibold uppercase text-[#9fb0cc]">
                <Video size={7} /> {item.clip}
              </span>
              {item.primary ? (
                <span className="absolute bottom-1 right-1 rounded-[2px] bg-[#083344]/90 px-1 py-px text-[7px] font-bold uppercase text-[#67e8f9]">
                  route
                </span>
              ) : null}
              <span className="absolute inset-0 grid place-items-center bg-black/45 opacity-0 transition-opacity group-hover:opacity-100">
                <span className="flex items-center gap-1 rounded-[4px] border border-accent-cyan/60 bg-[#05070f]/85 px-2 py-1 text-[9px] font-semibold text-[#67e8f9]">
            ...
            <div className="px-1.5 py-1">
              <div className="tnum flex items-center justify-between text-[8.5px]">
                <span className="font-mono font-semibold text-[#dbe6f5]">{item.time}</span>
                <span className="text-[#6d82a3]">{item.city}</span>
              </div>
              <div className="truncate text-[8px] text-[#7f93b3]">{item.location}</div>
              <div className="mt-[3px] flex items-center gap-1">
                <span className="truncate rounded-[2px] bg-[#0d1626] px-1 py-px text-[7px] text-[#8ea1c0] ring-1 ring-edge">
                  {item.tags[0]}
                </span>
                {item.watchlistHit ? (
                  <span className="flex shrink-0 items-center gap-0.5 rounded-[2px] bg-accent-red/20 px-1 py-px text-[7px] font-bold text-[#ff8b96]">
        ...
        {evidence.length === 0 ? (
          <div className="col-span-full grid place-items-center rounded-[5px] border border-dashed border-edge py-6 text-[10px] text-ink-dim">
