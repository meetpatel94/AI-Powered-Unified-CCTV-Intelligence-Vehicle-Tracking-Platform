      ...
      {alert ? (
        <>
          <div className="mb-1.5 flex items-center justify-between gap-2 border-b border-edge-soft pb-1.5">
            <span className="min-w-0 truncate text-[10px] font-semibold text-[#dbe6f5]">
              {alert.title} · <span className="tnum tracking-[0.03em] text-white">{alert.subject}</span>
            </span>
            <span className="tnum shrink-0 text-[8.5px] text-[#6d7f9e]">{alert.camera}</span>
          </div>
          <ol>
          ...
          <p className="mt-1 text-[8px] uppercase tracking-[0.08em] text-[#55668a]">
        ...
        <p className="text-[10px] text-ink-dim">No incident selected — click an alert card to trace its response log.</p>
      )}
    </Panel>
  );
}
