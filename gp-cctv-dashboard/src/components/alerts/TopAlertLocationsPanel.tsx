    ...
    >
      {topAlertLocations.map((location) => {
        ...
        return (
          ...
          >
            <span
              className={`tnum grid h-[16px] w-[16px] shrink-0 place-items-center rounded-[4px] text-[8.5px] font-bold ring-1 ${
            ...
            <span className="min-w-0 flex-1">
              <span className="flex items-center gap-1">
                ...
                <span className="truncate text-[9.5px] font-semibold text-[#dbe6f5]">{location.name}</span>
                <span className="truncate text-[8px] text-[#6d7f9e]">· {location.city}</span>
              </span>
            ...
            <span className="flex shrink-0 items-center gap-1.5">
              <span className="tnum hidden text-[7.5px] text-[#55668a] 2xl:inline">{location.peak}</span>
              <Trend size={9.5} strokeWidth={2.4} className={trendTone[location.trend]} />
              <span className="tnum text-[10px] font-bold text-white">{location.alerts}</span>
            </span>
          </div>
        );
      })}
