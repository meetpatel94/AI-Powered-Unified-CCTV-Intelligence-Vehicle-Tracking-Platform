...
export function DetailedReportDrawer({ snapshot, onClose, onExport }: DetailedReportDrawerProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <aside className="relative flex h-full w-[480px] animate-drawer-in flex-col border-l border-edge bg-[#0a1120] shadow-[0_0_40px_rgba(0,0,0,0.65)]">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-3.5 py-3">
          <section className="rounded-md border border-accent-purple/30 bg-[#160c22] px-3 py-2.5">
            <div className="flex items-center gap-1.5 text-[9px] font-semibold uppercase tracking-[0.1em] text-[#d0a4f7]">
              <FileText size={11} />
              Scope
            </div>
          ...
          <section className="mt-3">
            <h3 className="text-[9.5px] font-semibold uppercase tracking-[0.09em] text-ink-dim">Operational snapshot</h3>
          ...
          <section className="mt-3">
            <h3 className="text-[9.5px] font-semibold uppercase tracking-[0.09em] text-ink-dim">Narrative</h3>
            <p className="mt-1.5 rounded-md border border-edge bg-[#0c1424] px-3 py-2 text-[10.5px] leading-[16px] text-[#b9c7dd]">
          ...
          <section className="mt-3">
            <h3 className="text-[9.5px] font-semibold uppercase tracking-[0.09em] text-ink-dim">Key insights</h3>
            <ol className="mt-1.5 space-y-1.5">
              {insights.map((card) => (
                <li key={card.id} className="rounded-md border border-edge bg-[#0c1424] px-3 py-2">
                  <div className="text-[8.5px] font-semibold uppercase tracking-[0.1em] text-[#8ea1c0]">{card.kicker}</div>
                  <div className="mt-[2px] text-[11px] font-bold text-white">
                    {card.title}
                    <span className="tnum ml-2 text-accent-cyan">{card.metric}</span>
                  </div>
                  <p className="mt-1 text-[9.5px] leading-[14px] text-[#8ea1c0]">{card.body}</p>
                </li>
              ))}
            </ol>
          </section>
          ...
          <section className="mt-3">
            <h3 className="text-[9.5px] font-semibold uppercase tracking-[0.09em] text-ink-dim">Vehicle mix</h3>
            <ul className="mt-1.5 space-y-1">
              {vehicleTypes.map((slice) => (
                <li key={slice.id} className="flex items-center justify-between rounded-[4px] border border-edge bg-[#0c1424] px-2.5 py-1">
                  <span className="flex items-center gap-1.5 text-[10px] text-[#c3cfe2]">
                    <span className="h-1.5 w-1.5 rounded-full" style={{ background: slice.color }} />
                    {slice.label}
                  </span>
                  <span className="tnum text-[10.5px] font-bold text-white">{formatIn(slice.value)}</span>
                </li>
              ))}
            </ul>
          </section>
          ...
          <section className="mt-3">
            <h3 className="text-[9.5px] font-semibold uppercase tracking-[0.09em] text-ink-dim">Top locations</h3>
            <ol className="mt-1.5 space-y-1">
              {locations.map((location) => (
                <li key={location.id} className="flex items-center justify-between text-[10px]">
          ...
          <section className="mt-3 pb-1">
            <h3 className="text-[9.5px] font-semibold uppercase tracking-[0.09em] text-ink-dim">Unusual activity</h3>
            {unusual.length === 0 ? (
              <p className="mt-1.5 text-[10px] text-ink-dim">None in the current filter.</p>
            ) : (
              <ul className="mt-1.5 space-y-1.5">
                {unusual.map((event) => (
                  <li key={event.id} className="rounded-md border border-edge bg-[#0c1424] px-2.5 py-1.5 text-[10px] leading-[14px] text-[#c3cfe2]">
        ...
        <footer className="flex shrink-0 items-center gap-2 border-t border-edge px-3.5 py-2.5">
          ...
          <button
            type="button"
            onClick={onClose}
            className="flex h-[28px] flex-1 items-center justify-center rounded-[5px] border border-edge bg-panel text-[10px] font-medium text-[#c3cfe2] transition-colors hover:border-edge-strong hover:text-white"
          >
            Close
          </button>
        </footer>
