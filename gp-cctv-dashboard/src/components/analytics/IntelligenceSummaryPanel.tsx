...
export function IntelligenceSummaryPanel({
  ...
  return (
    <Panel
      ...
      action={
        <button
          ...
          className="flex h-[26px] items-center gap-1.5 rounded-[5px] border border-[#2f6fd0] bg-gradient-to-r from-[#1f5fd8] to-[#1a4fb5] px-2.5 text-[10px] font-semibold text-white shadow-[0_0_14px_-4px_rgba(47,125,255,0.8)] transition-all hover:brightness-110"
        >
          <FileText size={11} />
    ...
    >
      <div className="flex items-center justify-between text-[8.5px] text-[#6d82a3]">
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <span className={`text-[8px] font-semibold uppercase tracking-[0.1em] ${tone.kicker}`}>{card.kicker}</span>
              ...
              <p className="mt-1.5 line-clamp-3 text-[9px] leading-[13px] text-[#8ea1c0]">{card.body}</p>
            </article>
          );
        })}
      </div>
      ...
      <div className="min-h-0 flex-1 rounded-[6px] border border-edge bg-[#0c1424] px-2.5 py-2">
        <div className="mb-1.5 flex items-center justify-between">
          <span className="text-[9px] font-semibold uppercase tracking-[0.1em] text-[#d0a4f7]">Unusual activity indicators</span>
          <span className="tnum text-[8.5px] text-[#6d82a3]">{unusual.length} flags this window</span>
        </div>
        {unusual.length === 0 ? (
          <div className="py-2 text-[10px] text-ink-dim">No unusual indicators in the current filter.</div>
        ) : (
          <ul className="grid grid-cols-2 gap-x-4 gap-y-1.5">
            {unusual.map((event) => (
              <li key={event.id} className="flex min-w-0 items-start gap-2">
                <span className="tnum w-[34px] shrink-0 pt-[1px] text-[9px] font-semibold text-[#8ea1c0]">{event.time}</span>
                <span className={`mt-[5px] h-1.5 w-1.5 shrink-0 rounded-full ${unusualDot[event.tone]}`} />
                <span className="min-w-0 flex-1 text-[10px] leading-[14px] text-[#c3cfe2]">
                  {event.text}
                  {event.camera ? (
                    <button
                      type="button"
                      onClick={() => navigate(`/live-view?camera=${event.camera}`)}
                      className="ml-1.5 text-[9px] font-semibold text-accent-blue hover:text-cyan-300"
                    >
                      {event.camera}
                    </button>
                  ) : null}
