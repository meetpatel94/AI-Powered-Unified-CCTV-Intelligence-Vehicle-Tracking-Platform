...
export function ChartTip({ visible, x, y, title, rows }: ChartTipProps) {
  ...
  return (
    ...
    >
      <div className="text-[10.5px] font-semibold uppercase tracking-[0.08em] text-[#8ea1c0]">{title}</div>
      <ul className="mt-1 space-y-[2px]">
        {rows.map((row) => (
          <li key={row.label} className="flex items-center justify-between gap-3">
            <span className="flex items-center gap-1 text-[11.5px] text-[#9fb0cc]">
              {row.color ? (
                <span className="h-1.5 w-1.5 rounded-full" style={{ background: row.color, boxShadow: `0 0 6px ${row.color}` }} />
              ) : null}
              {row.label}
            </span>
            <span className="tnum text-[12px] font-bold text-white">{row.value}</span>
          </li>
        ))}
      </ul>
    </div>
