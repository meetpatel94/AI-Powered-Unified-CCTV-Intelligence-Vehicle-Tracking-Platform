...
export function MatchesOverTimePanel() {
  const points = matchesOverTime.map((point, index) => ({
    x: (index / (matchesOverTime.length - 1)) * 100,
    y: 100 - (point.value / MAX) * 100,
  }));
  const line = points.map((p) => `${p.x},${p.y}`).join(' ');
  const area = `0,100 ${line} 100,100`;
  const last = matchesOverTime[matchesOverTime.length - 1];
        ...
        <div className="relative min-w-0 flex-1">
          ...
          <div className="absolute inset-x-0 bottom-[16px] top-[6px]">
            ...
            {points.map((point, index) => (
              <span
                key={matchesOverTime[index].day}
                title={`${matchesOverTime[index].value} matches`}
