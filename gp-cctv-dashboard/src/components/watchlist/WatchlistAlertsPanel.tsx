...
function AlertRow({ alert }: { alert: WatchlistAlertItem }) {
  ...
  return (
    <button
      type="button"
      className={`flex w-full items-start gap-2 rounded-[5px] border border-l-[3px] px-2 py-[6px] text-left transition-colors hover:brightness-125 ${tone.shell}`}
    >
      <span className={`mt-[1px] grid h-[20px] w-[20px] shrink-0 place-items-center rounded-full ring-1 ${tone.iconBg}`}>
        <Icon size={11} strokeWidth={2.1} className={tone.icon} />
      </span>

      <span className="min-w-0 flex-1">
        <span className="flex items-start justify-between gap-2">
          <span className={`truncate text-[12px] font-semibold ${tone.title}`}>{alert.title}</span>
          <span className="tnum shrink-0 text-3xs text-ink-dim">{alert.time}</span>
        </span>
        {alert.label ? (
          <span className="mt-[1px] block truncate text-[12.5px] font-semibold tracking-wide text-white">
            {alert.label}
          </span>
        ) : null}
        <span className="mt-[1px] flex items-end justify-between gap-2">
          <span className="truncate text-[11px] text-[#94a5c2]">
