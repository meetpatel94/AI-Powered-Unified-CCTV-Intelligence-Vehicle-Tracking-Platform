...
export function AnprFeedPanel() {
  ...
  return (
    ...
    >
      <div className="mb-1 flex items-center gap-1 px-1 text-[10px] uppercase tracking-[0.08em] text-[#6d82a3]">
        <ScanLine size={9} />
      ...
      <ul className="min-h-0 flex-1 space-y-[3px] overflow-y-auto pr-0.5">
        {hits.map((hit, index) => (
          <li
            key={hit.id}
            className={`flex items-center gap-1 rounded-[3px] border px-1.5 py-[4px] text-[11.5px] transition-colors ${
              hit.watchlist
