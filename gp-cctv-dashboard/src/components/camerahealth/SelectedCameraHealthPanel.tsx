...
const actionBtn =
  'flex h-[26px] flex-1 items-center justify-center gap-1 rounded-[4px] border border-edge bg-panel-alt px-2 text-[10px] font-medium text-[#c3cfe2] transition-colors hover:border-edge-strong hover:text-white disabled:cursor-not-allowed disabled:opacity-45';
...
function stateChip(state: string, tone: 'green' | 'amber' | 'red' | 'cyan') {
  ...
  return (
    <span className={`rounded-[3px] border px-1 py-[1px] font-mono text-[9px] font-semibold uppercase ${styles[tone]}`}>{state}</span>
  );
}
      ...
      <Panel title="Selected Camera Health">
        <div className="grid h-full min-h-[220px] place-items-center px-4 text-center">
          <div>
            ...
            <p className="mt-2 text-[10.5px] text-ink-dim">Select a camera in the monitor grid</p>
    ...
    <Panel
      ...
      tools={
        <span className="flex items-center gap-1.5">
          <span className="tnum font-mono text-[9.5px] text-ink-faint">
    ...
    >
          ...
          {offline ? (
            <span className="absolute inset-0 grid place-items-center">
              <span className="flex flex-col items-center gap-1 rounded-[4px] border border-accent-red/40 bg-[#160709]/90 px-3 py-2">
                ...
                <span className="font-mono text-[9.5px] font-semibold uppercase tracking-[0.1em] text-[#ff8b96]">no video signal</span>
              </span>
            </span>
          ) : (
            <>
              ...
              <span className="absolute bottom-1.5 left-1.5 font-mono text-[9px] text-[#c3cfe2]">
                {camera.id} · {camera.location}
              </span>
              <span className="absolute right-1.5 bottom-1.5 font-mono text-[9px] text-[#9fc7ff]">{camera.city}</span>
            </>
          )}
        </div>

        <div className="flex items-center justify-between gap-2 border-t border-edge bg-[#0a1120] px-2 py-1">
          <span className="truncate font-mono text-[10px] font-semibold text-white">{camera.id}</span>
          <span className="truncate text-[9.5px] text-ink-dim">{camera.location}</span>
          <span className="ml-auto flex items-center gap-1.5">
            <span className="text-[9px] uppercase tracking-[0.08em] text-ink-faint">health</span>
      ...
      <div className="flex shrink-0 gap-1.5">
        <button
          ...
          className="flex h-[26px] flex-1 items-center justify-center gap-1 rounded-[4px] border border-[#2f6fd0] bg-gradient-to-r from-[#1f5fd8] to-[#1a4fb5] px-2 text-[10px] font-semibold text-white shadow-[0_0_12px_-4px_rgba(47,125,255,0.85)] transition-all hover:brightness-110 disabled:opacity-50"
        >
      ...
      <div className="shrink-0 rounded-[5px] border border-edge bg-[#0a1120] px-2 py-1.5">
        <div className="flex items-center justify-between">
          ...
          <span className="font-mono text-[9.5px]" style={{ color: toneInk[transport.tone] }}>
            {transport.label}
          </span>
        </div>
        ...
        <div className="mt-1.5 grid grid-cols-3 gap-1.5">
          <div className="rounded-[4px] border border-edge-soft bg-[#0b1222] px-1.5 py-1">
            <span className="block text-[8.5px] uppercase tracking-[0.08em] text-ink-faint">RTSP</span>
            {stateChip(
              camera.rtsp.state,
              camera.rtsp.state === 'connected' ? 'green' : camera.rtsp.state === 'timeout' ? 'red' : 'amber',
            )}
            <span className="mt-[2px] block truncate font-mono text-[8.5px] text-ink-faint" title={camera.rtsp.url}>
              {camera.rtsp.transport} · :554
            </span>
          </div>
          <div className="rounded-[4px] border border-edge-soft bg-[#0b1222] px-1.5 py-1">
            <span className="block text-[8.5px] uppercase tracking-[0.08em] text-ink-faint">WebRTC</span>
            {stateChip(camera.webrtc.state, camera.webrtc.state === 'active' ? 'green' : camera.webrtc.state === 'fallback' ? 'amber' : 'red')}
            <span className="mt-[2px] block truncate font-mono text-[8.5px] text-ink-faint">
              {camera.webrtc.latencyMs ? `${camera.webrtc.latencyMs} ms` : '—'} · {camera.webrtc.iceCandidate ?? 'no ICE'}
            </span>
          </div>
          <div className="rounded-[4px] border border-edge-soft bg-[#0b1222] px-1.5 py-1">
            <span className="block text-[8.5px] uppercase tracking-[0.08em] text-ink-faint">HLS</span>
      ...
      <div className="shrink-0 rounded-[5px] border border-edge bg-[#0a1120] px-2 py-1.5">
        <div className="flex items-center justify-between">
          <span className="text-[9.5px] font-semibold uppercase tracking-[0.09em] text-ink-dim">Stream telemetry</span>
          <span className="font-mono text-[9px] text-ink-faint">last 90 s</span>
        </div>
        ...
        <div className="mt-1.5 grid grid-cols-2 gap-2">
          <div className="rounded-[4px] border border-edge-soft bg-[#0b1222] px-1.5 py-1">
            <span className="flex items-center justify-between text-[8.5px] uppercase tracking-[0.08em] text-ink-faint">
              fps trend
      ...
      {/* AI / ANPR */}
      <div className="shrink-0 rounded-[5px] border border-edge bg-[#0a1120] px-2 py-1.5">
        <span className="text-[9.5px] font-semibold uppercase tracking-[0.09em] text-ink-dim">AI / ANPR pipeline</span>
      ...
      >
        <span className="text-[9.5px] font-semibold uppercase tracking-[0.09em]" style={{ color: toneInk[evaluation.tone] }}>
          {evaluation.attention ? 'Requires attention' : 'Subsystems nominal'}
        </span>
        <ul className="mt-1 space-y-[2px]">
          {evaluation.reasons.map((reason) => (
            <li key={reason} className="flex items-start gap-1.5 text-[9.5px] text-ink-dim">
      ...
      {detailsOpen ? (
        <div className="shrink-0 animate-fade-in rounded-[5px] border border-edge bg-[#0a1120] px-2 py-1.5">
          <button
            ...
            className="flex w-full items-center justify-between text-[9.5px] font-semibold uppercase tracking-[0.09em] text-ink-dim transition-colors hover:text-white"
          >
            Camera details
          ...
          <p className="mt-1 break-all rounded-[3px] border border-edge-soft bg-[#0b1222] px-1.5 py-1 font-mono text-[8.5px] text-ink-faint">
            {camera.streamUrl}
          </p>
        </div>
      ) : null}
