...
def get_cameras(db: Session = Depends(get_db)) -> list[CameraRead]:
    return [CameraRead.model_validate(c) for c in list_cameras(db)]


@router.get("/cameras/{camera_id}", response_model=CameraRead)
def get_camera_by_id(camera_id: str, db: Session = Depends(get_db)) -> CameraRead:
...
def ingest_catalogue(db: Session = Depends(get_db)) -> IngestResult:
